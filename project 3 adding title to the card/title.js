...
let addBtn =
  document.getElementById(
    "add-btn"
  ); /* grabbing the element whose id is btn, and adding the click event to the btn. if the btn is clicked then it runs the function. */
addBtn.addEventListener("click", function (e) {
  let addTxt = document.getElementById("addTxt");
  /* grabbing the textarea element whose id is addtxt.
   */
  let addTitle = document.getElementById("addTitle");
  /* grabbing the title element whose id is addTitle.
   */
  let notes = localStorage.getItem("notes"); //these returns the value of key whose name is notes(object). 

  if (notes == null) {// initially localstorage is empty so, there is no any notes(notes will be null).
    ...
    notesObj = JSON.parse(notes); // notesObj is an array of object.these gonna store the object at every index.
  }
  /* myObj is an object which contains title and the text*/
  myObj = {
      title: addTitle.value,
      text: addTxt.value
  }
  notesObj.push(myObj); // if anybody enter the text and the title then we are adding the text and the title in the object myObj and adding the object(myObj) as the element of noteObj array. here we are updating the array with the addTxt element.if anybody clicks on the addbtn, then we update the nodes, we add one more nodes.
