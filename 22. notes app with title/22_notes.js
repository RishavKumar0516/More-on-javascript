...
addBtn.addEventListener("click", function (e) {
   ...
   */
  /* grabbing the input element whose id is addTitle */
  let addTitle = document.getElementById("addTitle");
let notes = localStorage.getItem("notes"); //these returns the value of key whose name is notes(string), stored in local storage. agar  notes key pai koi value pahle sai local storage mai hai tou useea return ker do.
  if (notes == null) {
    ...
    notesObj = []; //if notes is null then setting the array as empty.
      ...
      /* notesObj is an array of objects. Each element in the array is object. */
    notesObj = JSON.parse(notes); // if notes is already exist then convert the notes(string) in object(array) using json.parse(). it takes a valid string and convert the string in to javascript object. so it convert the string(notes) in to array.
  }
  ...
  notesObj.push(myObj);//adding the myobj object as a element in the array notesobj .
   // if anybody enter the text then we are adding the text in the array notesObj. here we are updating the array with the addTxt element.if anybody clicks on the addbtn, then we update the nodes, we add one more nodes.
  ...
  addTitle.value = ""; // addtitle ke value ko blank ker daige, agar nahi karege tou uske andar likha hua humesha likha hi reh gaye ga.
...
function showNotes() {
  ...
  notesObj.forEach(function (element, index) {
    ...
    //notesObj is an array of object so, we are looping these. creating the function which takes element and index as argument.
